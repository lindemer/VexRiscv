...
case class Tor(previous : Tor) extends Area {
...
case class TorTranslatorPort(bus : MemoryTranslatorBus)

class TorPlugin(regions : Int, ioRange : UInt => Bool) extends Plugin[VexRiscv] with MemoryTranslator {
  ...
  val pmps = ArrayBuffer[Tor]()
  val dPorts = ArrayBuffer[TorTranslatorPort]()
  val iPorts = ArrayBuffer[TorTranslatorPort]()

  override def newTranslationPort(priority : Int, args : Any): MemoryTranslatorBus = {
    val port = TorTranslatorPort(MemoryTranslatorBus(new MemoryTranslatorBusParameter(0, 0)))
    priority match {
  ...
  override def build(pipeline: VexRiscv): Unit = {
    ...
    val core = pipeline plug new Area {
      ...
      for (region <- 0 until regions) {
        if (region == 0) pmps += Tor(null)
        else pmps += Tor(pmps.last)
