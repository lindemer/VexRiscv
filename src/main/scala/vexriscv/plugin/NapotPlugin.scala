...
case class NapotTranslatorPort(bus : MemoryTranslatorBus)

class NapotPlugin(regions : Int, ioRange : UInt => Bool) extends Plugin[VexRiscv] with MemoryTranslator {
  ...
  val pmps = ArrayBuffer[Napot]()
  val dPorts = ArrayBuffer[NapotTranslatorPort]()
  val iPorts = ArrayBuffer[NapotTranslatorPort]()

  override def newTranslationPort(priority : Int, args : Any): MemoryTranslatorBus = {
    val port = NapotTranslatorPort(MemoryTranslatorBus(new MemoryTranslatorBusParameter(0, 0)))
    priority match {
